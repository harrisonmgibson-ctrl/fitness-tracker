...
export default function StepGoal({ data, onChange, onNext, onBack }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label className="block text-sm text-[#997700] mb-1">Goal Weight (kg)</label>
        <input
      ...
      <div>
        <label className="block text-sm text-[#997700] mb-2">Weekly Goal</label>
        <div className="space-y-2">
          {WEEKLY_GOALS.map(g => (
            <button key={g.value} type="button"
              onClick={() => onChange({ weeklyGoalKg: g.value })}
              className={`w-full flex justify-between items-center px-4 py-3 rounded-xl transition-colors ${data.weeklyGoalKg === g.value ? 'bg-[#00AAFF] text-white' : 'bg-[#141414] border border-[#3D2E00] text-[#CCA800]'}`}>
      ...
      <div className="flex gap-3">
        <button type="button" onClick={onBack} className="flex-1 bg-[#1E1E1E] hover:bg-[#2A2A2A] text-[#CCA800] font-semibold py-3 rounded-xl transition-colors">
          Back
        </button>
        <button type="submit" className="flex-1 bg-[#00AAFF] hover:bg-[#0088DD] text-white font-semibold py-3 rounded-xl transition-colors">
          Next
        </button>
      </div>
    </form>
